...
[stem]
++++
\Psi(x,t) = \big(\frac{a}{\pi}\big)^{\frac{1}{4}}e^{-(\frac{a x^2}{2}+i\omega t) }
++++
...
. Calculate stem:[\big\langle -\frac{\partial V}{\partial x} \big\rangle]. Does Ehrenfest's theorem hold?
...
| Use a different wavefunction [in progress]
...
|Here are the first 20 digits of stem:[\sqrt{2}] ()
