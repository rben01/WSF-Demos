...
+
[{sublist-style}]
.. Apply the raising and lowering operators, ++\(L_+ = \hbar e^{i\phi}(\frac{\delta}{\delta \theta} + i\cot\theta \frac{\delta}{\delta \phi})\)++ and  ++\(L_- = -\hbar e^{-i\phi}(\frac{\delta}{\delta \theta} - i\cot\theta \frac{\delta}{\delta \phi})\)++, to find  ++\(Y_3^3\)++ and ++\(Y_3^1\)++.
...
.. Show that uncertainty
.. expectation value of n?


* {q-num} show max accuracy when m=1
