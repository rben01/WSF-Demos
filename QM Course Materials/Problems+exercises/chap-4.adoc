...
[%collapsible%open]
====
To compute ++\(\int_{-1}^1 p(x)P_n(x)\,dx\)++ using integration by parts, let
++\[\begin{align*}
u&=P_m(x)&dv&=P_n(x)\,dx\\
...
* {q-num} [I have not worked out this equation yet] Solve the spherical harmonic oscillator, ++\(V(r)=-k\lVert\mathbf r\rVert\)++.
