...
* {q-num} Suppose you have a hydrogen atom in the orbital angular momentum state ++\[Y = \frac{1}{2}Y_1^1 + \frac{1}{\sqrt{2}}Y_1^0+ \frac{1}{2}Y_1^{-1}\]++ Determine ++\(\braket{L^2}\)++, ++\(\braket{L_z}\)++,  ++\(\braket{L_x}\)++, and ++\(\braket{L_y}\)++.

* {q-num} Convert ++\(L^2\)++ from cartesian coordinates to spherical coordinates.

* {q-num} For the spherical harmonic ------   in
+
[{sublist-style}]
.. apply raising and lowering ops to find
.. verify 4.3.2 exp vals?
.. different wf
.. uncertainty
.. show l2 (l+ ) Y is eingenstate w same eigenval
.. satisfies ang eqn
...
=== Questions
* {q-num} What would the strength of the solenoid have to be for there to be maximum constructive interference of the electron beams. Destructive?
