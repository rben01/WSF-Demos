...
This wavefunction should be the sum of ++\(3!=6\)++ different terms (because ++\(3!\)++ is the number of ways to assign the [.no-break]#++\(\v r\)++’s# to the [.no-break]#++\(\psi\)++’s#) and should have the property that swapping any two particles — any two indices ++\(m,n\in\{1,2,3\}\)++ — should negate the resulting wavefunction.
...
.. Using the "`count the two-fermion swaps`" principle from part (c), how would the sign of the wavefunction change under the following four-particle swaps?
... ++\((1,2,3,4) \to (2,1,4,3)\)++
... ++\((1,2,3,4) \to (4,3,2,1)\)++
... ++\((1,2,3,4) \to (4,1,2,3)\)++
...
As you saw above, if you know the sign of a permutation, you know how the sign of the wavefunction will change when that permutation is used to relabel the fermions in the system: the resulting sign change is just the sign of the permutation.
+
...
+
Show that the _Slater determinant_ shown below (where ++\(A\)++ is once again a normalization constant) satisfies the properties of the wavefunction of an ++\(N\)++-fermion system: it contains all ++\(N!\)++ assignments of states to particles and is antisymmetric under exchange of any two of the ++\(N\)++ fermions.
++\[\begin{align*}
...
.. Derive the Pauli exclusion principle from the Slater determinant.
Which property of the determinant leads to the Pauli exclusion principle?
...
* {q-num} Show that ... full wavefunction (spin and spatial) for indistinguisahble boson, fermion, and distinguishable particles
