...
\Psi_b(\vec{r_2})\)++ with  energies ++\(a\)++ and ++\(b\)++ form a system with total energy ++\(E=a+b\)++. For ++\(E=4\)++ determine all degenerate wavefunctions, ++\(\Psi_{ab}(\vec{r_1}, \vec{r_2})\)++, if the particles are...
+
[{sublist-style}]
. Distinguishable
. Bosons
