...
In terms of the original wavefunction, what is the new wavefunction? UPDATE THIS ONE AS DISCUSSED AT ASRC
+
.Solution
[%collapsible]
====
...
\Psi_b(\vec{r_2})\)++ with  energies ++\(a\)++ and ++\(b\)++ form a system with total energy ++\(E=a+b\)++.


ith a system with total energy ++\(E=a+b\)++ and one particle in state ++\(
\Psi_a(\vec{r_1})\)++ with energy ++\(a\)++ and a second particle in the state ++\(\Psi_b(\vec{r_2})\)++ with energy ++\(b\)++, write the  for ++\(E=4\)++ if the particles are...
+
[{sublist-style}]
. Distinguishable
. Bosons
