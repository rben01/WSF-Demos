...
. Normalize ++\(\Psi(x,0)\)++
. Solve for ++\(F(k)\)++
. Write the integral for ++\(\Psi(x,t)\)++

* *2.4.{counter:qn}* Using the same steps as in 2.4.1, find ++\(\Psi(x,t)\)++ for ++\(\Psi(x,0) = \)++
+
NOTE: In this case, you can perform the final integral for ++\(\Psi(x,t)\)++.
