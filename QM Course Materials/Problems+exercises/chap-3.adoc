...
Supposing ++\(\kappa^*\)++ is _not_ an eigenvalue of ++\(A^\dagger\)++, then there would be no nonzero ++\(f\)++ for which ++\((A^\dagger-\kappa^* I)f=0\)++, from which we may conclude that ++\(A^\dagger-\kappa^* I\)++ is invertible; call its inverse ++\(B\)++. Then ++\((A^\dagger-\kappa^* I)B=I\)++, and so ++\(I=I^\dagger=B^\dagger(A^\dagger-\kappa^* I)^\dagger = B^\dagger (A -\kappa I)\)++.
So ++\(A-\kappa I\)++ is also invertible, which means ++\(\kappa\)++ is not an eigenvalue of ++\(A\)++.
...
Indeed, ++\(T\)++ as defined above has no inverse — it shifts functions one unit to the left, but since the domain ends at 0, there is no way to "un-shift" the portion of the function on ++\([0,1)\)++; it's just lost.
--
====
