...
* *3.2.{counter:qn}* Show that ++\(Q^{\dagger\dagger}=Q\)++ for any operator ++\(Q\)++. (In other words, show that the adjoint of the adjoint is the original operator.)
+
.Solution
[%collapsible]
====
...
* *2.3.{counter:qn}* Find the eigenfunctions and corresponding eigenvalues of the operator ++\(\frac{d}{dx^n}\)++ for positive integer ++\(n\)++.
(Hint: the only eigenfunction of ++\(\frac{d^1}{dx^1}\)++ is ++\(f(x)=e^x\)++.)
