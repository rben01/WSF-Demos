...
.. Find a continuous function (on any domain) that is absolutely integrable, but not square-integrable.
.. Find a continuous function (on any domain) that is square-integrable, but not absolutely integrable.
...
[{sublist-style}]
. One example is the following function defined on the positive reals:
++\[\begin{align*}
f(x)=\begin{cases}
x^{-\frac{1}{2}}-1&x\le1\\
0&x>1
...
. One example is the following function defined on the reals:
++\[\begin{align*}
f(x)=\begin{cases}
\frac{1}{|x|}&|x|\ge1\\
1&|x|<1
...
\Psi&=\int_{\mathcal D} c_t f_t\,dt\quad \textrm{(continuous spectrum)}\\
\end{align*}\]++
...
NOTE: This is exactly why we can say that ++\(c_\alpha =\langle f_\alpha | \Psi\rangle\)++ tells you how much of ++\(f_\alpha\)++ is in ++\(\Psi\)++. Because the inner project with a normalized vector ++\(u\)++ is equvalent to a projection onto ++\(u\)++, the inner product picks out just the component corresponding to ++\(u\)++.
