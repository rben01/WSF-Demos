...
====
[{sublist-style}]
.. Since ++\(x\)++ and ++\(y\)++ are orthogonal, ++\(\langle x|y\rangle=\langle y|x\rangle=0\)++. Now we simply apply the linearity of the inner product:
++\[\begin{align*}
...
Consider the statement ++\(\|x\|\ge \frac{|\langle y|x\rangle|}{\|y\|}\)++.
If we define ++\(y'=\frac{y}{\|y\|}\)++ to be ++\(y\)++ normalized, and define ++\(P_{y'}=|y'\rangle\langle y'|\)++ to be the projection operator onto ++\(y'\)++, then
++\[\begin{align*}
\Bigl\|P_{y'}|x\rangle\Bigr\|&=\Bigl\||y'\rangle\langle y'|x\rangle\Bigr\|\\
&=\frac{|\langle y|x\rangle|}{\|y\|}\Bigl\||y'\rangle\Bigr\|\\
&=\frac{|\langle y|x\rangle|}{\|y\|}
\end{align*}\]++
