...
The matrix exponential ++\(e^A\)++ shares some properties with the ordinary exponential ++\(e^z\)++ over complex numbers (or, conversely, with respect to the exponential, matrices themselves share several properties with complex numbers):
+
[{sublist-style}]
