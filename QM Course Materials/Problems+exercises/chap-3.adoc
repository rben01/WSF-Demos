...
++\[\begin{align*}
\Bigl\|P_{y'}\Ket{x}\Bigr\|&=\Bigl\|\bra{y'}\Braket{y'|x}\Bigr\|\\
&=\frac{\left|\Braket{y|x}\right|}{\|y\|}\Bigl\|\Ket{|y'}\Bigr\|\\
&=\frac{\left|\Braket{y|x}\right|}{\|y\|}
\end{align*}\]++
So all the Cuachy-Schwarz Inequality says is that ++\(\left\|P_{y'}\Ket{x}\right\|\le\left\|x\right\|\)++, i.e. the norm of the projection of a vector ++\(x\)++ onto some other vector cannot exceed the norm of ++\(x\)++ itself, which on its face is obvious (how could a vector get longer by taking only a portion of it?).
