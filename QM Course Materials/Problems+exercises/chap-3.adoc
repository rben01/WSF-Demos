...
* {q-num} Show that ++\(e^{\langle x|}=e^x\)++, i.e., the operator exponential of the ++\(\hat x\)++ observable is multiplication by ++\(e^x\)++.

* {q-num} Show that the solution to the differential equation ++\(\frac{d}{dt}|x(t)\rangle= A|x(t)\rangle\)++ with initial condition ++\(|x(0)\rangle=|x_0\rangle\)++, where the operator ++\(A\)++ is constant, is ++\(|x(t)\rangle=e^{At}|x_0\rangle\)++.
+
