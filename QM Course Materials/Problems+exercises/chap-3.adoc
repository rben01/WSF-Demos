...
* [[Cauchy-Schwarz,3.1.4]]{q-num} Let's prove the Cauchy-Schwarz Inequality: ++\(\left|\Braket{x|y}\right|^2\le \Braket{x|x}\Braket{y|y}\)++, or equivalently ++\(\left|\Braket{x|y}\right|\le \|x\|\|y\|\)++ ("`the magnitude of the inner product is at most the product of the norms.`").
...
&=\|z\|^2+\frac{|\Braket{y|x}|^2}{\|y\|^2}\\
&\ge \frac{|\Braket{y|x}|^2}{\|y\|^2}
\end{align*}\]++
Therefore,
++\[\begin{align*}
|\Braket{x|y}^2|\le \|x\|^2\|y\|^2=\Braket{x|x}\Braket{y|y}
\end{align*}\]++
as desired.
+
...
&=\Braket{f_\alpha|\Psi}\ \Ket{f_\alpha}\\
&=c_\alpha \Ket{f_\alpha}
\end{align*}\]++

[NOTE]
--
This is exactly why we can say that ++\(c_\alpha =\Braket{f_\alpha | \Psi}\)++ tells you how much of ++\(f_\alpha\)++ is in ++\(\Psi\)++. Because the inner project with a normalized vector ++\(u\)++ is equvalent to a projection onto ++\(u\)++, the inner product picks out just the component corresponding to ++\(u\)++.
...
It states that a sufficient condition for ++\(\Braket{Q}\)++ to be constant over time is that ++\(\Braket{[\hat H,\hat Q]}=0\)++ and ++\(\frac{d\Braket{Q}}{dt}=0\)++ (or, in particular, ++\(\Braket{Q}=0\)++).
+
[{sublist-style}]
.. Using this fact, when is ++\(\Braket{x}\)++ conserved?
...
In other words, ++\(\Braket p\)++ is conserved whenever the potential is constant over all space.
We would expect this result; in a flat potential, objects feel no net force, so their momentum won't change.
====
...
+
You can assume the *binomial theorem*: if ++\(\binom{n}{k}:=\frac{n!}{k!(n-k)!}\)++ represents the number of ways to choose a ++\(k\)++-element subset from an ++\(n\)++-element set, and ++\(x\)++ and ++\(y\)++ are arbitrary numbers, then
++\[\begin{align*}
...
++\[\begin{align*}
\Braket{f|e^A g}&=\Braket{f|\sum_{n=0}^\infty \frac{A^n}{n!} g}\\
&=\sum_{n=0}^\infty\Braket{f|A^n g}\\
&=\sum_{n=0}^\infty\Braket{A^n f|g}\\
&=\Braket{\sum_{n=0}^\infty A^n f| g}\\
&=\Braket{e^A f| g}
\end{align*}\]++
as desired.
...
. With ++\(Q\)++ hermitian, we'll show that ++\(e^{iQ}(e^{iQ})^\dagger=I\)++.
First,
++\[\begin{align*}
e^{iQ}(e^{iQ})^\dagger&=e^{iQ}e^{(iQ)^\dagger}&\textrm{(part (b))}\\
&=e^{iQ}e^{-iQ}
\end{align*}\]++
Now, ++\(iQ\)++ and ++\(-iQ\)++ commute — the scalars commute with everything and ++\(Q\)++ commutes with itself — so by part (d) we have
++\[\begin{align*}
e^{iQ}e^{-iQ}=e^{iQ + (-iQ)} = e^\bf 0
\end{align*}\]++
which, by part (a), is the identity operator.
====
