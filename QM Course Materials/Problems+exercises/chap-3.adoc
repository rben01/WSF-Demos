...
When is ++\(f\)++ an eigenfunction  of ++\(Q_1+Q_2\)++, ++\(Q_1Q_2\)++, and ++\(Q_2Q_1\)++?
...
[%collapsible]
====
Unsurprisingly, the eigenfunctions of powers of the differential operator are linear combinations of exponentials.
...
[sidebar]
--
This problem highlights the fact that the eigenfunctions of an operator depend on the _precise_ operator in question; ++\(\frac{d^n}{dx^n}-q\)++ is quite different from ++\(\frac{d^n}{dx^n}\)++ when ++\(q\ne0\)++.
--
====
