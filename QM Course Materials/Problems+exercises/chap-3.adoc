...
* [[Cauchy-Schwarz,3.1.4]]{q-num} Let's prove the Cauchy-Schwarz Inequality: ++\(|\Braket{x|y}|^2\le \Braket{x|x}\,\Braket{y|y}\)++, or equivalently ++\(|\Braket{x|y}|\le \|x\|\|y\|\)++ ("`the magnitude of the inner product is at most the product of the norms.`").
...
\end{align*}\]++
Multiplying both sides by ++\(\|y\|^2\)++ and using the fact that ++\(|\Braket{y|x}|=|\Braket{x|y}|\)++, we obtain
++\[\|x\|^2\|y\|^2\ge |\Braket{x|y}|^2\]++
as desired.
+
Let's think about what this actually means.
Consider the statement ++\(\|x\|\ge \frac{|\Braket{y|x}|}{\|y\|}\)++.
If we define ++\(y'=\frac{y}{\|y\|}\)++ to be ++\(y\)++ normalized, and define ++\(P_{y'}=|y'\rangle\langle y'|\)++ to be the projection operator onto ++\(y'\)++, then
++\[\begin{align*}
\Bigl\|P_{y'}|x\rangle\Bigr\|&=\Bigl\||y'\rangle\Braket{y'|x}\Bigr\|\\
&=\frac{|\Braket{y|x}|}{\|y\|}\Bigl\||y'\rangle\Bigr\|\\
&=\frac{|\Braket{y|x}|}{\|y\|}
\end{align*}\]++
So all the Cuachy-Schwarz Inequality says is that the norm of the projection of a vector ++\(x\)++ onto some other vector cannot exceed the norm of ++\(x\)++ itself, which on its face is obvious (how could a vector get longer by taking only a portion of it?).
...
&=\Braket{f_\alpha|\Psi}\ |f_\alpha\rangle\\
...
++\[
|a\rangle\Braket{a|x}=P|x\rangle=\lambda_0|x\rangle=|0\rangle
\]++
++\(|a\rangle\ne|0\rangle\)++, so ++\(\Braket{a|x}=0\)++.
...
|a\rangle\Braket{a|x}=P|x\rangle=\lambda_1|x\rangle=|x\rangle
\]++
