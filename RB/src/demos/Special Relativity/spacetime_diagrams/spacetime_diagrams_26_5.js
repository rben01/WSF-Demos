...
function update({ speed }) {
	...
	const elems = svg
		...
		.data(d => {
			const index = d.index;
			const y0 = (index / (AXES.y.max - AXES.y.min)) * 0.8;
			const slope = speed;
			const { min: _x1, max: _x2 } = AXES.x;
			const x1 = _x1 * 0.8;
			const x2 = _x2 * 0.8;
			const [y1, y2] = [x1, x2].map(x => slope * x + y0);

			const textPos = { x: xScale(x2) - 15, y: yScale(y2) - 10 };
			if (textPos.y < 20) {
