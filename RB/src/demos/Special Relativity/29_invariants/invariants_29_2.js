...
const AXIS_MARGINS = {
	top: 0.05,
	bottom: 0.05,
	left: sliders.displacement.x !== null ? 0.05 : 0.05,
	right: sliders.displacement.x !== null ? 0.05 : 0.1,
};
...
const textSpans = {
	...
	upright: {
		...
		xSq: document.getElementById("text-upright-x-sq"),
		ySq: document.getElementById("text-upright-y-sq"),
	},
	transformed: {
		x: document.getElementById("text-transformed-x"),
		y: document.getElementById("text-transformed-y"),
		xSq: document.getElementById("text-transformed-x-sq"),
		ySq: document.getElementById("text-transformed-y-sq"),
	},
...
function getLinesData({
	...
	const rXAx = Math.sqrt(
		Math.pow(tfAxPosEndCoords.xAx.x, 2) + Math.pow(tfAxPosEndCoords.xAx.y, 2),
	);
	const cosThetaXAx = tfAxPosEndCoords.xAx.x / rXAx;
	const sinThetaXAx = -tfAxPosEndCoords.xAx.y / rXAx;

	const rYAx = Math.sqrt(
		Math.pow(tfAxPosEndCoords.yAx.x, 2) + Math.pow(tfAxPosEndCoords.yAx.y, 2),
	);
	...
	const tfAxesData = [
		...
		{
			shape: "line",
			class: "tf-axis-x-arrow",
			attrs: {
				x1: xScale(tfAxPosEndCoords.xAx.x),
				y1: yScale(tfAxPosEndCoords.xAx.y),
				x2: xScale(tfAxPosEndCoords.xAx.x) + arrowDist * cosThetaXAx,
				y2: yScale(tfAxPosEndCoords.xAx.y) + arrowDist * sinThetaXAx,
				...tfAxesAttrs,
		...
		{
			shape: "line",
			class: "tf-axis-y-arrow",
			attrs: {
				x1: xScale(tfAxPosEndCoords.yAx.x),
				y1: yScale(tfAxPosEndCoords.yAx.y),
				x2: xScale(tfAxPosEndCoords.yAx.x) + arrowDist * cosThetaYAx,
				y2: yScale(tfAxPosEndCoords.yAx.y) + arrowDist * sinThetaYAx,
				...tfAxesAttrs,
				"marker-end": `url(#${ARROWHEAD_IDS.transformed})`,
			},
		},
...
function updateAngle({
	...
	textSpans.upright.xSq.innerHTML = fmtFloat(Math.pow(x, 2), 2);
	textSpans.upright.ySq.innerHTML = fmtFloat(Math.pow(y, 2), 2);

	textSpans.transformed.xSq.innerHTML = fmtFloat(Math.pow(tfCoords.xAxXCoord, 2), 2);
	textSpans.transformed.ySq.innerHTML = fmtFloat(Math.pow(tfCoords.yAxYCoord, 2), 2);

	const dist = Math.pow(x, 2) + Math.pow(y, 2);
	for (let i = 0; i < textSpans.squareds.length; ++i) {
		textSpans.squareds[i].innerHTML = fmtFloat(dist, 2);
	}
