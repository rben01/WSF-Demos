const W = 1450,
  ...
  y_axis = graph
    ...
    .call(d3.axisLeft(y).tickValues([20, 40, 60, 80])),
  wave = graph
    .append("path")
    .style("fill", "none")
    .style("stroke", "#5df")
...
function a2(chi, k, w) {
  return math.divide(
    math.multiply(
      math.sum(-1, math.exp(math.multiply(math.i, math.multiply(chi, 4 * w)))),
      math.multiply(math.subtract(k, chi), math.sum(k, chi))
    ),
    math.multiply(
      math.exp(math.multiply(math.i, 2 * w * k)),
      math.subtract(
        math.multiply(
          math.exp(math.multiply(math.i, math.multiply(chi, 4 * w))),
...
function a3(chi, k, w) {
  return math.divide(
    math.multiply(-2 * k, math.sum(k, chi)),
    math.multiply(
      math.exp(math.multiply(math.i, math.multiply(w, math.subtract(k, chi)))),
      math.subtract(
        math.multiply(
