const w = 1450,
  ...
  x_axis = graph
    ...
    .call(d3.axisBottom(x).tickValues([-10, -5, 5, 10])),
  y_axis = graph
    .append("g")
    .attr("class", "axis")
    .attr("transform", "translate(" + w / 2 + ", 0)")
    .call(d3.axisLeft(y).tickValues([-10, -5, 5, 10, 15, 20])),
  wave = graph
    .append("path")
    .style("fill", "none")
    .style("stroke", "#5df")
...
function chi(m, e, d) {
  return math.sqrt(2 * m * (e + d));
}

function a2(chi, k, w2) {
  return math.divide(
    math.multiply(
      math.sum(-1, math.exp(math.multiply(math.i, 4 * w2 * chi))),
      (k - chi) * (k + chi)
    ),
    math.multiply(
...
function update() {
  ...
  well_outline.attr(
    "d",
    `M ${x(-w2)} ${y(0)} L ${x(-w2)} ${y(-d)} L ${x(w2)} ${y(-d)} L ${x(
      w2
    )} ${y(0)}`
  );
}
